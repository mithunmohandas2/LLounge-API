...
export const tutorJwtAuth = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        if (decode.status !== 200 || decode?.role !== 'tutor') return res.status(401).json({ message: 'Unauthorized Access' })
